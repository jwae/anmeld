...
function actionLabel(action: string) { if (action === "NEU") return "NEU"; if (action === "UPDATE") return "UPDATE"; return action; }
      ...
      <table class="wizard-table">
        ...
        <tbody>
          <tr v-for="row in rows" :key="row.row_number" :class="`status-${row.status}`">
            ...
            <td :class="{ 'cell-changed cell-changed-green': hasFieldChanged(row, 'geburtsdatum') }" :title="fieldTitle(row, 'geburtsdatum')">{{ fieldValue(row, 'geburtsdatum') }}</td>
            ...
            <td><span class="status-chip" :class="statusChipClass(row)">{{ row.status === "fehler" ? "FEHLER" : row.status === "warnung" ? "WARNUNG" : actionLabel(row.import_action) }}</span></td>
